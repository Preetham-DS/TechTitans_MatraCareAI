...
import { getRiskAssessments, getPatientEntries } from '../firebase/firestoreService';
...
const Dashboard = ({ user, onStartNew }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      // Try Firestore first when user is logged in
      if (user?.uid) {
        try {
          const [fbAssessments, fbEntries] = await Promise.all([
            getRiskAssessments(user.uid),
            getPatientEntries(user.uid),
          ]);

          // Merge with local data so offline entries aren't lost
          const localData = loadPatientData();
          const mergedEntries = [...fbEntries, ...localData.entries]
            .filter((v, i, arr) => arr.findIndex(x => x.id === v.id) === i)
            .sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
          const mergedHistory = [...fbAssessments, ...localData.riskHistory]
            .filter((v, i, arr) => arr.findIndex(x => x.id === v.id) === i)
            .sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));

          const merged = { entries: mergedEntries, riskHistory: mergedHistory, alerts: localData.alerts };
          setPatientData(merged);

          if (mergedEntries.length >= 2) {
            const trend = predictFutureTrend(mergedEntries);
            if (trend) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-12 fade-in">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3 flex-wrap justify-end">
          <button 
            ...
            className="flex items-center justify-center space-x-2 bg-blue-600 text-white hover:bg-blue-700 px-6 py-3 rounded-full font-bold shadow-sm transition-all hover-lift"
          >
            <QrCode className="w-5 h-5" />
            <span>Share with Health Worker</span>
          </button>
          <button 
      ...
      {/* FEATURE 1: IMPACT METRICS (PROOF OF VALUE) */}
      <div className="mb-10 fade-in">
        <h3 className="text-xl font-bold text-gray-800 mb-4 flex items-center gap-2">
          <Activity className="w-5 h-5 text-primary-dark" /> Impact Metrics
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-2xl p-5 border border-green-200 shadow-sm hover:shadow-md transition-all">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-green-800 uppercase tracking-wide">Emergencies Prevented</p>
                <p className="text-3xl font-black text-green-900 mt-1">3</p>
              </div>
            </div>
          </div>
          <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-2xl p-5 border border-blue-200 shadow-sm hover:shadow-md transition-all">
            <div className="flex items-center gap-3">
              <span className="text-2xl">📊</span>
              <div>
                <p className="text-sm font-semibold text-blue-800 uppercase tracking-wide">High-Risk Early Detections</p>
                <p className="text-3xl font-black text-blue-900 mt-1">5</p>
              </div>
            </div>
          </div>
          <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-2xl p-5 border border-purple-200 shadow-sm hover:shadow-md transition-all">
            <div className="flex items-center gap-3">
              <span className="text-2xl">⏱</span>
              <div>
                <p className="text-sm font-semibold text-purple-800 uppercase tracking-wide">Avg Response Time Reduced</p>
                <p className="text-3xl font-black text-purple-900 mt-1">60%</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <SchemeMatcher patientData={patientData.entries[patientData.entries.length - 1]} />
      ...
      {isQrOpen && <GenerateQR onClose={() => setIsQrOpen(false)} />}
    </div>
  );
};
