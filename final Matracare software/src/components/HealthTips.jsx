...
const HealthTips = () => {
  ...
  const [sortedTips, setSortedTips] = useState(TIPS);
  ...
  useEffect(() => {
    ...
    if (latestEntry) {
      const { systolic, diastolic, glucose, hemoglobin, symptoms = [], highBP, diabetes, habits = {} } = latestEntry;
      
      let activeSymptoms = [];
      if (Array.isArray(symptoms)) {
        activeSymptoms = symptoms;
      } else if (typeof symptoms === 'object' && symptoms !== null) {
        // Map new object structure back to old IDs for compatibility with tip scoring
        activeSymptoms = Object.entries(symptoms)
          .filter(([k, v]) => v !== 'none')
          .map(([k, v]) => {
            if (k === 'headache') return 'severe_headache';
            if (k === 'vision') return 'blurred_vision';
            if (k === 'fetalMovement') return 'reduced_fetal_movement';
            return k;
          });
      }

      const priorityScores = {};
      TIPS.forEach(tip => priorityScores[tip.id] = 0);

      // Condition logic for personalized tips
      if (systolic >= 140 || diastolic >= 90 || highBP) {
        priorityScores[2] += 10; // Monitor BP
        priorityScores[8] += 5;  // Danger signs
      }
      if (glucose >= 140 || diabetes) {
        priorityScores[3] += 10; // Nutrition
      }
      if (hemoglobin > 0 && hemoglobin < 11) {
        priorityScores[3] += 8;  // Nutrition (iron)
      }
      if (activeSymptoms.includes('reduced_fetal_movement')) {
        priorityScores[6] += 15; // Monitor fetal movement
        priorityScores[8] += 10; // Danger signs
      }
      if (activeSymptoms.includes('severe_headache') || activeSymptoms.includes('blurred_vision') || activeSymptoms.includes('bleeding')) {
        priorityScores[8] += 15; // Danger signs
        priorityScores[5] += 5;  // Rest
      }
      if (habits.drankWater === false) {
        priorityScores[1] += 10; // Hydration
      }
      if (activeSymptoms.includes('dizziness') || activeSymptoms.includes('swelling')) {
        priorityScores[5] += 10; // Rest
        priorityScores[2] += 5;  // Monitor BP
      }

      const newSortedTips = [...TIPS].sort((a, b) => priorityScores[b.id] - priorityScores[a.id]);
      setSortedTips(newSortedTips);
    }
  }, []);

  const lang = (SECTION_LABELS[language] ? language : 'en');
  const sl = SECTION_LABELS[lang];
  const visible = expanded ? sortedTips : sortedTips.slice(0, 3);
