...
const RiskForm = ({ onCalculate, prefillData, onPrefillApplied }) => {
  ...
  const [symptomsObj, setSymptomsObj] = useState(saved.symptomsObj || {
    headache: 'none',
    vision: 'none',
    swelling: 'none',
    dizziness: 'none',
  ...
  useEffect(() => {
    saveLastFormInput({ ...vitals, symptomsObj, highBP, diabetes });
  }, [vitals, symptomsObj, highBP, diabetes]);
  ...
  const handleVoiceData = (data) => {
    ...
    if (data.symptoms && typeof data.symptoms === 'object') {
      setSymptomsObj(prev => ({ ...prev, ...data.symptoms }));
    }
  };
  ...
  const handleVoiceInput = () => {
    ...
    recognition.onresult = (event) => {
      ...
      if (detected.length > 0) {
        const newSymptoms = { ...symptomsObj };
        detected.forEach(d => {
          // Map older voice keywords to new symptom keys if needed, default to moderate
          const key = d === 'severe_headache' ? 'headache' : d === 'blurred_vision' ? 'vision' : d === 'reduced_fetal_movement' ? 'fetalMovement' : d;
          newSymptoms[key] = 'moderate'; 
        });
        setSymptomsObj(newSymptoms);
  ...
  const handleSymptomChange = (id, level) => {
    setSymptomsObj(prev => ({ ...prev, [id]: level }));
  };
  ...
  return (
    <div id="assessment" className="max-w-3xl mx-auto bg-white rounded-3xl p-6 md:p-10 shadow-xl border border-primary-light">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        {/* ── Symptoms (Severity Support) ── */}
        <div>
          <h3 className={`font-semibold text-gray-700 mb-4 border-b pb-2 ${easyMode ? 'text-xl' : 'text-lg'}`}>
            {easyMode ? '🤒 How do you feel?' : 'Symptoms'}
          </h3>

          <div className={`grid gap-4 ${easyMode ? 'grid-cols-1 sm:grid-cols-2' : 'grid-cols-1'}`}>
            {SYMPTOMS.map((symptom) => {
              const currentLevel = symptomsObj[symptom.id] || 'none';
              return (
                <div key={symptom.id} className="bg-gray-50 border border-gray-100 rounded-2xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-sm hover:shadow-md transition-shadow">
                  <div className="flex items-center gap-3">
                    {easyMode ? <span className="text-3xl">{symptom.emoji}</span> : symptom.icon}
                    <span className="font-bold text-gray-700">{easyMode ? symptom.easyLabel[language] : symptom.label}</span>
                  </div>
                  
                  <div className="flex bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm self-start md:self-auto w-full md:w-auto">
                    {['none', 'mild', 'moderate', 'severe'].map(level => {
                      const isActive = currentLevel === level;
                      let activeClass = 'bg-gray-200 text-gray-800 font-bold';
                      if (isActive) {
                        if (level === 'mild') activeClass = 'bg-yellow-400 text-yellow-900 font-bold shadow-inner';
                        if (level === 'moderate') activeClass = 'bg-orange-500 text-white font-bold shadow-inner';
                        if (level === 'severe') activeClass = 'bg-red-600 text-white font-bold shadow-inner';
                      }
                      ...
                      return (
                        <button
                          ...
                          className={`flex-1 md:flex-none px-3 py-2 md:px-4 md:py-2 text-xs md:text-sm transition-colors border-r last:border-r-0 border-gray-200 ${isActive ? activeClass : 'hover:bg-gray-50 text-gray-600 font-medium'}`}
                        >
