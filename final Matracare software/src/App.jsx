...
function AppInner() {
  ...
  const calculateRisk = async (data) => {
    ...
    // Save to Firestore if user is logged in (Do not await to prevent UI blocking)
    if (user?.uid) {
      saveRiskAssessment(user.uid, {
        ...data,
        level: result.level,
        score: result.score,
      ...
      <main className="flex-grow w-full flex flex-col">
        {currentView === 'home' ? (
          <div className="w-full">
            ...
            <section className="py-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full" id="assessment">
              {!user ? (
                <div className="text-center py-12 bg-primary-light/50 rounded-3xl border border-primary-light fade-in">
                  ...
                  <button
                    ...
                    className="bg-pink-700 hover:bg-pink-800 text-white px-8 py-3 rounded-full text-lg font-semibold shadow-lg transition-all"
                  >
                    Login / Sign Up
                  </button>
                </div>
                ...
                <div className="w-full">
                  ...
                  {isCalculating && (
                    <div className="flex flex-col items-center justify-center py-20 fade-in w-full">
                  ...
                  {riskLevel && !isCalculating && (
                    <div id="result-section" className="space-y-8 pt-8 w-full">
          ...
          <div className="w-full max-w-6xl mx-auto"><Dashboard user={user} onStartNew={() => setCurrentView('home')} /></div>
        ) : currentView === 'profile' ? (
          <div className="w-full max-w-4xl mx-auto"><Profile user={user} onUpdateUser={handleUpdateUser} onBack={() => { setIsNewUser(false); setCurrentView('home'); }} isFirstTime={isNewUser} /></div>
        ) : currentView === 'triage' ? (
          <div className="w-full max-w-6xl mx-auto"><TriagePortal onBack={() => setCurrentView('home')} /></div>
        ) : null}
      </main>
