 ...
 * - For PDFs: uses FileReader to get text (basic — works for text-based PDFs)
 * - For images: creates an image, then uses basic brightness-based approach
 *   (In production, this would use Tesseract.js or a backend OCR)
 */
const readFileAsText = (file) => {
  return new Promise((resolve) => {
    const reader = new FileReader();
    if (file.type === 'application/pdf') {
      // For PDF files, read as text (only works for text-embedded PDFs)
      reader.onload = (e) => resolve(e.target.result || '');
      reader.readAsText(file);
    } else {
      // For images, read as text won't work — we return empty and show image for manual entry
      reader.onload = (e) => resolve(e.target.result || '');
      reader.readAsText(file);
    }
  });
