...
import { saveLastFormInput, loadLastFormInput } from '../utils/storage';
  ...
  const handleVitalChange = (e) => {
    ...
    if (name === 'age') {
      ...
      if (value === '') {
        ...
        setAgeError('Age must be at least 18 years for maternal health assessment.');
      } else if (age > 50) {
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {[
              { key: 'age', placeholder: 'e.g. 28', min: 18, max: 50, step: 1, required: true },
        ...
        <button
          type="submit"
          className={`w-full bg-primary-dark hover:bg-primary-dark/90 text-white rounded-2xl font-bold shadow-lg transition-all hover:shadow-xl transform hover:-translate-y-0.5 ${easyMode ? 'py-5 text-2xl' : 'py-4 text-lg'}`}
        >
          {easyMode ? `✅ ${hl.submit}` : 'Evaluate Risk Level'}
        </button>
      </form>
    </div>
  );
